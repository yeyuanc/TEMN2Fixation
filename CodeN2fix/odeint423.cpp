	...
	if (dt <= pow(0.5,maxit))
	  ...
	  mflag = 1;
          if (nintmon == 0)
          {
            ...
            //if (i = 71){printf("odient 65: oldstate=%.4f,pstate=%.3f\n",oldstate[i], pstate[i]);}
          }
	  ++nintmon;  //if (i = 71){printf("odient 67: nintom=%i",nintmon);}
	}

        if (test == ACCEPT)
        {
          for(i = 0; i < numeq;i++) { pstate[i] = dum4[i]; }
          //if (i = 71){printf("odient 73: pstate=%.3f,dum4=%.3f\n",pstate[i],dum4[i]);}
          time += dt;
        ...
        if (nintmon == maxitmon)
        {
          ...
          //if (i = 71){printf("odient 85: nintmon=%i,maxitmon=%i,pstate=%.3f,oldstate=%.3f\n",nintmon,maxitmon,pstate[i],oldstate[i]);}
        }
      }
    }    /* end rkf integrator */
  }      /* end time while */
...
void Odeint4::rkf(const int& numeq, double pstate[], double& pdt, const int& pdm)
{
  ...
  for (i = 0; i < numeq;i++)
  {
    dum4[i] = dum5[i] = pstate[i];  //if (i = 71){printf("odient 158: pstate=%.3f,dum5=%.3f,dum4=%.3f\n",pstate[i],dum5[i],dum4[i]);}
  ...
  step(numeq,dum5,rk45,dum5,pdt);  //if (i = 71){printf("odient 201: dum5=%.3f,dum4=%.3f\n",dum5[i],dum4[i]);} 
  for (i = 0; i < numeq; i++)
  {
    error[i] = fabs(dum4[i] - dum5[i]); //if (i = 71){printf("odient 204: dum5=%.3f,dum4=%.3f\n",dum5[i],dum4[i]);}
  }
			 ...
			 double& pdt)
{
  ...
  for (int i = 0; i < numeq; i++)
  {
    ...
    //if (i = 72){printf("odient 219: pstate=%.3f\n",pstate[i]);}
  }
