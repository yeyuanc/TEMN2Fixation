...
// added for reading Density value
  ...
  #include "datdens.cpp"  //TEMelmnt uses the Phdata class
#endif
...
************************************************************* */
     ...
     int temgisin(ofstream& flog1, int& ftlerr, const int& atmsflag,
		  ...
		  FILE* ftair, FILE* fprec, FILE* fvap, FILE* flulc, FILE* fnpp,FILE* fnhxdepo,FILE* fnoydepo,
...
************************************************************* */
  ...
  private:
     ...
     int loadteprec(FILE* fprec, Clmdata prec, const int& numspin,
                    const int& spintime, const int& RTIME, int& ftlerr,
		    ofstream& flog1);
     int loadtenhxdepo(FILE* fnhxdepo, Clmdata nhxdepo,
                         const int& numspin, const int& spintime,
                         const int& RTIME, int& ftlerr, ofstream& flog1);
     int loadtenoydepo(FILE* fnoydepo, Clmdata noydepo,
                         const int& numspin, const int& spintime,
                         const int& RTIME, int& ftlerr, ofstream& flog1);
