...
************************************************************** */
  ...
  tem.y[tem.I_VEGCBG] = initstat[itype][tem.I_VEGCBG].mon[CYCLE-1];
  tem.y[tem.I_VEGCAG] = initstat[itype][tem.I_VEGCAG].mon[CYCLE-1];
  ...
  for (dm = 0; dm < CYCLE; dm++)
  {
    ...
    tem.totalc[dm] = 0.0;
    tem.veg.planta[dm].carbon = tem.y[tem.I_VEGCAG];
    tem.veg.plantb[dm].carbon = tem.y[tem.I_VEGCBG];
...
//N deposition 
int TEMelmnt::loadtenhxdepo(FILE* fnhxdepo, Clmdata nhxdepo,
                         const int& numspin, const int& spintime,
                         const int& RTIME, int& ftlerr, ofstream& flog1)
{
...
// Get ndepo from transient data set
...
//  nondepo = 0;
  ...
  {
    k = i - totspin;
    gisend = nhxdepo.getdel(fnhxdepo);
    if (gisend == -1) { return gisend; }
    ftlerr = coregerr(flog1,"TEMVEG", col, row, "nhx", nhxdepo.col, nhxdepo.row);

// Determine NDEPO for equilibrium conditions

    if ( k == 0)
    {
      for (j = 0; j < CYCLE; j++) { tem.atms.tnhxdepo[0][j] = nhxdepo.mon[j]; }
      tem.atms.nhxdepoyear[0] = nhxdepo.year - totspin;
      tem.atms.yrtnhxdepo[0] = nhxdepo.total;
//      if (ndepo.total <= 0.0) { nondepo = 1; }
    }

// Determine NDEPO for transient conditions

    else
    {
      for (j = 0; j < CYCLE; j++) { tem.atms.tnhxdepo[i][j] = nhxdepo.mon[j]; }
      tem.atms.nhxdepoyear[i] = nhxdepo.year;
      tem.atms.yrtnhxdepo[i] = nhxdepo.total;
    }
  }

// Determine NDEPO during spin up
  ...
  for (i = 0; i < numspin; i++)
  {
    ...
    {
      k = (i * spintime) + j + 1;
      tem.atms.nhxdepoyear[k] = tem.atms.nhxdepoyear[0] + k;
      tem.atms.yrtnhxdepo[k] = tem.atms.yrtnhxdepo[dyr];
      for (dm = 0; dm < CYCLE; dm++)
      {
        tem.atms.tnhxdepo[k][dm] = tem.atms.tnhxdepo[dyr][dm];
      }
      ++dyr;
    }
  }
...
//noy 
int TEMelmnt::loadtenoydepo(FILE* fnoydepo, Clmdata noydepo,
                         const int& numspin, const int& spintime,
                         const int& RTIME, int& ftlerr, ofstream& flog1)
{
...
//  nondepo = 0;
  ...
  {
    k = i - totspin;
    gisend = noydepo.getdel(fnoydepo);
    if (gisend == -1) { return gisend; }
    ftlerr = coregerr(flog1,"TEMVEG", col, row, "noy", noydepo.col, noydepo.row);

// Determine NDEPO for equilibrium conditions

    if ( k == 0)
    {
      for (j = 0; j < CYCLE; j++) { tem.atms.tnoydepo[0][j] = noydepo.mon[j]; }
      tem.atms.noydepoyear[0] = noydepo.year - totspin;
      tem.atms.yrtnoydepo[0] = noydepo.total;
//      if (ndepo.total <= 0.0) { nondepo = 1; }
    }
...
// Determine NDEPO for transient conditions
    ...
    {
      for (j = 0; j < CYCLE; j++) { tem.atms.tnoydepo[i][j] = noydepo.mon[j]; }
      tem.atms.noydepoyear[i] = noydepo.year;
      tem.atms.yrtnoydepo[i] = noydepo.total;
    }
  }
...
// Determine NDEPO during spin up
  ...
  for (i = 0; i < numspin; i++)
  {
    ...
    {
      k = (i * spintime) + j + 1;
      tem.atms.noydepoyear[k] = tem.atms.noydepoyear[0] + k;
      tem.atms.yrtnoydepo[k] = tem.atms.yrtnoydepo[dyr];
      for (dm = 0; dm < CYCLE; dm++)
      {
        tem.atms.tnoydepo[k][dm] = tem.atms.tnoydepo[dyr][dm];
      }
      ++dyr;
    }
  }
...
int TEMelmnt::temgisin(ofstream& flog1, int& ftlerr, const int& atmsflag,
		       const int& itype, const int& maxtype,
                       ...
                       FILE* fstxt, FILE*felev, FILE* fph, FILE* fdens, FILE* fnirr, FILE* fpar,
		       FILE* ftair, FILE* fprec, FILE* fvap,FILE* fnhxdepo,FILE* fnoydepo, FILE* flulc, FILE* fnpp,
...
//added for nhx deposition    
    if (tem.atms.tnhxdepoflag == 1)
    {
      gisend = loadtenhxdepo(fnhxdepo, nhxdepo, numspin, spintime, RTIME, ftlerr, flog1);
      if (gisend == -1) { return gisend; }
      for (i = 0; i < CYCLE; i++) { tem.atms.nhxdepo[i] = tem.atms.tnhxdepo[0][i]; }
    }
    else
    {
      gisend = nhxdepo.getdel(fnhxdepo);
      if (gisend == -1) { return gisend; }
      ftlerr = coregerr(flog1,"TEMVEG", col, row, "nhx", nhxdepo.col, nhxdepo.row);
      tem.atms.yrnhxdepo = nhxdepo.total;
//      if (ndepo.total <= 0.0) { nondepo = 1; }
      for (i = 0; i < CYCLE; i++) { tem.atms.nhxdepo[i] = nhxdepo.mon[i]; }
    }
//added for nyo deposition    
    if (tem.atms.tnoydepoflag == 1)
    {
      gisend = loadtenoydepo(fnoydepo, noydepo, numspin, spintime, RTIME, ftlerr, flog1);
      if (gisend == -1) { return gisend; }
      for (i = 0; i < CYCLE; i++) { tem.atms.noydepo[i] = tem.atms.tnoydepo[0][i]; }
    }
    else
    {
      gisend = noydepo.getdel(fnoydepo);
      if (gisend == -1) { return gisend; }
      ftlerr = coregerr(flog1,"TEMVEG", col, row, "noy", noydepo.col, noydepo.row);
      tem.atms.yrnoydepo = noydepo.total;
//      if (ndepo.total <= 0.0) { nondepo = 1; }
      for (i = 0; i < CYCLE; i++) { tem.atms.noydepo[i] = noydepo.mon[i]; }
    }
                            ...
                            const int& ntempred, const int& natmspred)
{
  ...
  for (i = 0; i < ntempred; i++)
  {
    ...
    else if (strcmp(predname[k],tem.predstr[tem.I_VEGCAG]) == 0)
    {
      for (dm = 0; dm < CYCLE; dm++)
      {
	     tempred.mon[dm] = tem.veg.planta[dm].carbon;  // VEGCAG
      }
    }
    else if (strcmp(predname[k],tem.predstr[tem.I_VEGCBG]) == 0)
    {
      for (dm = 0; dm < CYCLE; dm++)
      {
	     tempred.mon[dm] = tem.veg.plantb[dm].carbon;  // VEGCBG
      }
    }    
...
/*    else if (strcmp(predname[k],tem.predstr[tem.I_N2O]) == 0)
    ...
    else if (strcmp(predname[k],tem.predstr[tem.I_N2O]) == 0)
    {
      for (dm = 0; dm < CYCLE; dm++)
      {
  		 tempred.mon[dm] = tem.microbe.N2O[dm] * 1000.0;  // N2O
      }
    }    
    else if (strcmp(predname[k],tem.predstr[tem.I_N2]) == 0)
    {
      for (dm = 0; dm < CYCLE; dm++)
      {
     	 tempred.mon[dm] = tem.microbe.N2[dm] * 1000.0;  // N2
      }
    }  
    ...
    else if (strcmp(predname[k],tem.predstr[tem.I_N2FIX]) == 0)
    {
      for (dm = 0; dm < CYCLE; dm++)
      {
   	    tempred.mon[dm]= tem.microbe.N2Fix[dm] * 1000.0;  // N2FIX
      }
    }     
    else if (strcmp(predname[k],tem.predstr[tem.I_N2FIXS]) == 0)
    {
      for (dm = 0; dm < CYCLE; dm++)
      {
	     	tempred.mon[dm] = tem.microbe.N2Fixs[dm] * 1000.0;  // N2FIXS
      }
    }   
    else if (strcmp(predname[k],tem.predstr[tem.I_N2FIXFL]) == 0)
    {
      for (dm = 0; dm < CYCLE; dm++)
      {
	     	tempred.mon[dm] = tem.microbe.N2Fixfl[dm] * 1000.0;  // N2FIXFL
      }
    }       
    ...
    // Write output data to files
		if (strcmp(predname[k],tem.predstr[tem.I_NOX]) == 0
			//|| strcmp(predname[k],tem.predstr[tem.I_N2]) == 0
			//|| strcmp(predname[k],tem.predstr[tem.I_N2O]) == 0
      ...
      //|| strcmp(predname[k],tem.predstr[tem.I_N2FIX]) == 0
      //|| strcmp(predname[k],tem.predstr[tem.I_N2FIXS]) == 0
      //|| strcmp(predname[k],tem.predstr[tem.I_N2FIXFL]) == 0)
			{
