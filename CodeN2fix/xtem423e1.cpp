...
const int MAXPRED = 92+4+10+8+1+6+7; // changed from 85 to 92 by DWK on 20000202, QZ, plus 4+10 //add +8 for N cycle by YYL //add +1 for DOC by YYL
//added 6 by YYuan N2O added 7 by YYuan for N2 fix
const int MAXGRID = 1;
...
//YY added for mpi
  ...
  fptname(fnminout.fnmnhxdepo,dir_label);
	fnhxdepo=fopen(fnminout.fnmnhxdepo,"r");
 	if (fnhxdepo==NULL) {
	cout <<"Can not open file "<<fnminout.fnmnhxdepo<<endl;
 		exit(-1); 
	}   
  fptname(fnminout.fnmnoydepo,dir_label);
	fnoydepo=fopen(fnminout.fnmnoydepo,"r");
 	if (fnoydepo==NULL) {
	cout <<"Can not open file "<<fnminout.fnmnoydepo<<endl;
 		exit(-1); 
	}   
   //ndepo YYuan
...
//  double totamnt[NUMEQ+2];
  ...
  for (i = 1; i < MAXGRID; i++)
  {
    ...
    if (tmflgs.temflag == 1)
    {
      ...
      telmnt[i].tem.atms.tnhxdepoflag = telmnt[0].tem.atms.tnhxdepoflag;
      telmnt[i].tem.atms.tnoydepoflag = telmnt[0].tem.atms.tnoydepoflag;
...
//Vegetation specific TEM parameters for microbes
	...
	telmnt[i].tem.microbe.fco2k[j] = telmnt[0].tem.microbe.fco2k[j];
 	telmnt[i].tem.microbe.npot[j] = telmnt[0].tem.microbe.npot[j];
  telmnt[i].tem.microbe.toptl[j] = telmnt[0].tem.microbe.toptl[j];
  telmnt[i].tem.microbe.topth[j] = telmnt[0].tem.microbe.topth[j];  
  telmnt[i].tem.microbe.wuph[j] = telmnt[0].tem.microbe.wuph[j];  
  telmnt[i].tem.microbe.fnup[j] = telmnt[0].tem.microbe.fnup[j];  
  telmnt[i].tem.microbe.kc[j] = telmnt[0].tem.microbe.kc[j];
  telmnt[i].tem.microbe.fplant[j] = telmnt[0].tem.microbe.fplant[j];  
  telmnt[i].tem.microbe.rfl[j] = telmnt[0].tem.microbe.rfl[j];  
	...
	if (end1 == -1) { exit(0); }
	  ...
	  if (tmflgs.temflag == 1 && fatalerr == 0)
	    ...
	    end1 = telmnt[0].temgisin(flog1,fatalerr,tmflgs.atmsflag,
	     	                      ...
	     	                      fstxt,felev, fph, fdens,
                                      fnirr,fpar,
                                      ftair,fprec,fvap, fnhxdepo, fnoydepo,
                                      flulc,fnpp,
                                      fkdin,fstate,
                                      tmflgs.numspin,tmflgs.spintime,tmflgs.RTIME);
            if (end1 == -1) { exit(0); }
...
/// N deposition  
  telmnt[0].tem.atms.tnhxdepoflag = 0;  
  if (tmflgs.equil == 0)
  {
    cout << "Do you have nhx deposition data?:" << endl;
    cout << "Enter 0 for No:" << endl;
    cout << "Enter 1 for Yes: ";
    fpara >> telmnt[0].tem.atms.tnhxdepoflag;

    flog1 << endl << "Do you have transient nhx deposition data?:" << endl;
    flog1 << "Enter 0 for No:" << endl;
    flog1 << "Enter 1 for Yes: " << endl;
    flog1 << "temnt[0].tem.atms.tnhxdepoflag = " << telmnt[0].tem.atms.tnhxdepoflag << endl << endl;
  }

  cout << "Please enter the name of the file containing the monthly nhx deposition data: " << endl;
  cout << "               (e.g., NHXDEPO.GIS) " << endl;
  //cin >> ifilename;
  fpara >> fnminout.fnmnhxdepo;
  
  flog1 << "Please enter the name of the file containing the monthly nhx deposition data: " << endl;
  flog1 << "               (e.g., NHXDEPO.GIS) " << endl;
  flog1 << fnminout.fnmnhxdepo << endl << endl;
  
  
  
  telmnt[0].tem.atms.tnoydepoflag = 0;  
  if (tmflgs.equil == 0)
  {
    cout << "Do you have noy deposition data?:" << endl;
    cout << "Enter 0 for No:" << endl;
    cout << "Enter 1 for Yes: ";
    fpara >> telmnt[0].tem.atms.tnoydepoflag;

    flog1 << endl << "Do you have transient noy deposition data?:" << endl;
    flog1 << "Enter 0 for No:" << endl;
    flog1 << "Enter 1 for Yes: " << endl;
    flog1 << "temnt[0].tem.atms.tnoydepoflag = " << telmnt[0].tem.atms.tnoydepoflag << endl << endl;
  }
  
  cout << "Please enter the name of the file containing the monthly noy deposition data: " << endl;
  cout << "               (e.g., NOYDEPO.GIS) " << endl;
  //cin >> ifilename;
  fpara >> fnminout.fnmnoydepo;
  
  flog1 << "Please enter the name of the file containing the monthly noy deposition data: " << endl;
  flog1 << "               (e.g., NOYDEPO.GIS) " << endl;
  flog1 << fnminout.fnmnoydepo << endl << endl;
