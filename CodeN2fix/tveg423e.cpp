                         ...
                         double& prveetmx, double& prvleaf)
{
  ...
  normeet = eet / prveetmx;  //printf("tveg 45: normeet=%.3f,eet=%.3f,prveetmx=%.3f\n",normeet,eet,prveetmx);
  unnormleaf = (aleaf[dcmnt] * normeet) + (bleaf[dcmnt] * prvleaf) 
               + cleaf[dcmnt];  
               //printf("tveg 47: dcmnt=%i,unnormleaf=%.3f,normeet=%.3f,prvleaf=%.3f\n",dcmnt,unnormleaf,normeet,prvleaf);
  if (unnormleaf < (0.5 * minleaf[dcmnt]))
  {
    unnormleaf = 0.5 * minleaf[dcmnt]; 
    //printf("tveg 50: dcmnt=%i, unnormleaf=%.3f,minleaf=%.3f\n",dcmnt,unnormleaf, minleaf[dcmnt]);
  }
...
************************************************************** */
  ...
  gpp  = co2 * gv;  //printf("tveg 234: gpp=%.3f,co2=%.3f,gv=%.3f\n", gpp , co2, gv);
  gpp *= cmax * foliage / (kc[dcmnt] + gpp);  ///printf("tveg 235: gpp=%.3f,cmax=%.3f,foliage=%.3f,kc=%.3f\n", gpp,cmax,foliage,kc[dcmnt]);
  gpp *= leaf * par / (ki[dcmnt] + par); //printf("tveg 236: gpp=%.3f,leaf=%.3f,par=%.3f,ki=%.3f\n", gpp,leaf,par,ki[dcmnt]);
  gpp *= temp;   //printf("tveg237: gpp=%.3f,temp=%.3f\n", gpp,temp);

  gpp *= thawpercent; // addition by Q.Z.   
  //printf("tveg242: gpp=%.3f,thawpercent=%.3f\n", gpp,thawpercent);
...
// calculate the root-shoot ratio    YYuan 033024 
double Tveg42::rsratio(int& dcmnt, double& tair, double& vegc)
{   
   
  double vegca; // aboveground C
  
  if (dcmnt>1)
  {
    if (dcmnt == 2 || dcmnt ==3) {vegca = vegc / (1+exp(-0.043 * tair + 1.01));} 
    if (dcmnt == 7 || dcmnt ==8 || dcmnt ==12) {vegca = 0.01437 * pow (vegc,1.4128);}
    if (dcmnt == 10) {vegca = 0.60756 * pow (vegc,1.025);}  
    if (dcmnt == 4 || dcmnt == 5) { vegca = pow(10, -0.177 + 0.00052 * tair + 1.021 * log10(vegc));} 
    if (dcmnt ==6 || dcmnt ==9 || dcmnt ==11) { vegca = pow(10, -0.2293 + 0.0006429 * tair + 1.031 * log10(vegc));} 
  }
  
  //printf("tveg 265: dcmnt=%.i, vegcb=%.3f, tair=%.3f, vegc=%.3f\n",dcmnt, vegcb,tair,vegc);
  if (vegca > vegc) {vegca=vegc;}
  return  vegca;
};
/* *************************************************************
************************************************************* */
...
************************************************************** */
  ...
  kr = exp((kra[dcmnt]*vegc) + krb[dcmnt]); //printf("tveg385: kr=%.3f, kra=%.3f,vegc=%.3f,krb=%.3f\n",kr,kra[dcmnt],vegc,krb[dcmnt]);
  rmaint  = kr * vegc;  //printf("tveg386: rmaint=%.3f, kr=%.3f,vegc=%.3f\n",rmaint,kr,vegc);
  rmaint *= respq10;  //printf("tveg387: rmaint=%.3f, respq10=%.3f\n",rmaint,respq10);
